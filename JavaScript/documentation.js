...
function init(doNotCreateCaptionsAndLists, enableFullWidth) {
  ...
  createTableToolTips();
  if (enableFullWidth) {
    rootNode.style.maxWidth = window.innerWidth + "px";
  }
}
...
function createContents() {
  ...
  rootNode.insertBefore(contents, rootNode.firstChild);
  ...
  for (var i = 0; i < rootNodeChilds.length; i++) {
    switch(rootNodeChilds[i].nodeName.toUpperCase()) {
    case "H1":
    case "H2":
    case "H3":
    case "H4":
    case "H5":
    case "H6":
      createNewContentsEntry(rootNodeChilds[i].nodeName, rootNodeChilds[i]);
      break;
    }
  }
}
...
function createTitle() {
  ...
  rootNode.insertBefore(div, rootNode.firstChild);
...
function initVariables() {
  rootNode = window.document.getElementById("documentation");
  rootNodeChilds = rootNode.childNodes;
...
function createListOfFigures() {
  ...
  rootNode.insertBefore(listOfFigures, contents.nextSibling);
...
function createListOfTables() {
  ...
  rootNode.insertBefore(listOfTables, contents.nextSibling);
